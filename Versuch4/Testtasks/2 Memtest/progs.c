...
char tt_performTest(uint8_t testId, uint8_t iterations, uint16_t max, bool errorsAsHex, const char *testName, MEMTEST((*test)));
...
void tt_showPhase(uint8_t i, const char *name);
...
void program2(void) {
    ...
    while (startedProcs != CONCURRENCY_PROCS) continue;
...
char tt_performTest(uint8_t testId, uint8_t iterations, uint16_t max, bool errorsAsHex, const char *testName, MEMTEST((*test))) {
    ...
    // diversity of tests
    const Factor factors[] = {1, 7, 13, 29, 101, 127};
    const uint8_t factorSize = sizeof(factors) / sizeof(factors[0]);
    uint32_t errors = 0;
...
MEMTEST(tt_addressBits) {
    ...
    const uint8_t addressBit = (round & 1) ? (ADDRESS_BITS - cell - 1) : cell;
    const uint8_t pattern = addressBit * f ? addressBit * f : 0xFF;
